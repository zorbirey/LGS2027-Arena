(() => {
  'use strict';
  const BUILD_ID='20260824-13',LEGAL='Türkiye Yüzyılı Maarif Modeli dikkate alınmıştır.';
  window.LGS_ARENA_CONFIG=Object.freeze({
    schemaVersion:11,appVersion:'6.5.1-zeus-visual-premium',buildId:BUILD_ID,season:2027,appName:'LGS 2027 Arena',coverLocked:true,
    coverStandard:'LGS2027-ZEUS-ARENA-COVER-2026-08-23-V630-20260824-13',coverAsset:'assets/zeus-hero-20260823-02.webp?v='+BUILD_ID,
    ...
    accessModel:Object.freeze({freeDailyQuestions:50,rewardedAdsPerDay:6,freeUnlockHour:8,zeusPremium:true,smartNotesPremium:true,parentPremium:true,preferencePremium:true}),legalNotice:LEGAL
  });
  const core=document.createElement('link');core.rel='stylesheet';core.href='./visual-core-v5.css?v='+BUILD_ID;core.dataset.arenaVisualCore='6.5.1';document.head.appendChild(core);
  const layout=document.createElement('link');layout.rel='stylesheet';layout.href='./layout-v52.css?v='+BUILD_ID;layout.dataset.arenaLayout='6.5.1';document.head.appendChild(layout);
  function premium(){try{const state=JSON.parse(localStorage.getItem('lgsArenaPwaV02')||'{}'),until=Date.parse(state.freePremiumUntil||'');return !!state.isPremium||(Number.isFinite(until)&&until>Date.now())}catch{return false}}
