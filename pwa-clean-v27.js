...
function init(){var menu=document.getElementById('menuBtn');if(menu){menu.title='Uygulamayı yükle';menu.setAttribute('aria-label','LGS Arena uygulamasını yükle');menu.onclick=install}if('serviceWorker' in navigator&&location.protocol!=='file:'){navigator.serviceWorker.register('./service-worker.js?v=27',{scope:'./'}).then(function(reg){reg.update().catch(function(){})}).catch(function(){toast('Çevrimdışı servis bu açılışta kurulamadı. Uygulama çevrimiçi çalışmaya devam eder.')})}}
