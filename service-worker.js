...
const CRITICAL=[
  './index.html','./manifest.webmanifest','./styles.css','./mobile-v04.css','./clean-v27.css',
  './data/matematik.js','./data/fen.js','./data/turkce.js','./data/inkilap.js','./data/din.js','./data/notes.js',
  './data/english-v18.js','./data/english-notes-v18.js','./data/bank-v011.js','./data/adaptive-bank-v17.js',
  './entry-clean-v27.js','./core-v23.js','./pwa-clean-v27.js','./assets/zeus.webp'
];

const OPTIONAL=[
  './android16-v05.css','./android16-v06.css','./android16-v07.css','./v09-vibrant.css','./v10-zeus-fix.css',
  './parent-tracking.css','./lgs-scoring.css','./weekly-exam-v18.css',
  './parent-tracking.js','./lgs-scoring.js','./weekly-exam-v18.js',
...
async function fetchRequired(url){
  ...
  for(let i=0;i<4;i++){
    try{
      ...
      return r;
    }catch(e){last=e;if(i<3)await sleep(500+i*700)}
  }
  throw last||new Error('Fetch failed '+url);
}

async function putRequired(cache,url){
  const r=await fetchRequired(url);
...
self.addEventListener('install',event=>{
  event.waitUntil((async()=>{
    ...
    await Promise.all(CRITICAL.map(url=>putRequired(cache,url)));
    await Promise.allSettled(OPTIONAL.map(url=>putOptional(cache,url)));
...
async function refresh(request,key){
  try{
    ...
    if(r&&r.ok){const cache=await caches.open(CACHE_NAME);await cache.put(key||request,r.clone())}
  }catch(e){}
}
...
self.addEventListener('fetch',event=>{
  ...
  event.respondWith((async()=>{
    ...
    if(cached){event.waitUntil(refresh(request,request));return cached}
    try{
